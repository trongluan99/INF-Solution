...
public class InfAdjust {
    ...
    public static void setEventNamePurchase(String eventNamePurchase) {
        InfAdjust.eventNamePurchase = eventNamePurchase;
    }
    ...
    static void logPaidAdImpressionValue(double revenue, String currency) {
        if (InfAd.getInstance().getAdConfig().getAdjustConfig() != null && InfAd.getInstance().getAdConfig().getAdjustConfig().isEnableAdjust()) {
            AdjustEvent event = new AdjustEvent(InfAd.getInstance().getAdConfig().getAdjustConfig().getEventAdImpression());
